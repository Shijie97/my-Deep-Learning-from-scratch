...
network_weight_decay = MultiLayerNetExtend(input_size = input_size,
                              hidden_size_list = hidden_size_list,
                              output_size = output_size,
                              activation = activation,
                              weight_init_std = weight_init_std,
...
for i in range(iter_num):
    ...
    if i % iter_num_per_epoch == 0:
        ...
        train_acc_network_weight_decay = network_weight_decay.accuracy(x_train, t_train)
        train_acc_weight_decay_list.append(train_acc_network_weight_decay)

        test_acc_network_weight_decay = network_weight_decay.accuracy(x_test, t_test)
...
plt.plot(x, train_acc_list, label = 'without decay for train', marker = 'o')
plt.plot(x, train_acc_weight_decay_list, label = 'with decay for train', marker = 's')
plt.plot(x, test_acc_list, label = 'without decay for test', marker = 'v', linestyle = '--')
plt.plot(x, test_acc_weight_decay_list, label = 'with decay for test', marker = '^', linestyle = '--')
plt.ylim(0, 1)
plt.title('compare between without decay and with decay')
plt.xlabel('epoch')
plt.ylabel('accuracy')
